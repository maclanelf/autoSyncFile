...
async function refresh(jobId: number) {
  ...
  for (const item of (transferred.transferred || []) as RcloneTransfer[]) {
    ...
    upsertTransferFile({jobId, path: item.name, size: item.size || 0, bytes: item.bytes ?? item.size ?? 0, status: item.error ? "failed" : "completed", error: item.error, startedAt: item.startedAt || now, finishedAt: item.completedAt || now});
  }
...
export async function GET(request: Request, {params}: {params: Promise<{id: string}>}) { try { const {id} = await params; const job = await refresh(Number(id)); const query = new URL(request.url).searchParams; const state = query.get("state") === "transferring" ? "transferring" : query.get("state") === "finished" ? "finished" : undefined; const page = Math.max(1, Number(query.get("page")) || 1); return NextResponse.json({...job, ...listTransferFiles(Number(id), state, page)}); } catch (error) { return NextResponse.json({error: error instanceof Error ? error.message : String(error)}, {status: 502}); } }
